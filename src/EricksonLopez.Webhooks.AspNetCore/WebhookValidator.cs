...
public sealed partial class WebhookValidator : IWebhookValidator
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            if (headers.TryGetValue(WebhookHeaders.Signature, out var ericksonSigs))
            {
        ...
        {
            if (headers.TryGetValue(WebhookHeaders.StandardSignature, out var stdSigs))
            {
        ...
        if (_options.Protocol is WebhookReceiverProtocol.EricksonLopez or WebhookReceiverProtocol.AutoDetect)
        {
            if (headers.TryGetValue(WebhookHeaders.Timestamp, out var ericksonTimes))
            {
        ...
        {
            if (request.Headers.TryGetValue(WebhookHeaders.StandardTimestamp, out var stdTimes))
            {
        ...
        var diff = unchecked(currentSeconds - timestampSeconds);
        var skewSeconds = diff == long.MinValue ? long.MaxValue : Math.Abs(diff);
        ...
        if (request.Headers.TryGetValue(WebhookHeaders.StandardDeliveryId, out var idVal))
        {
            webhookId = GetFirstNonEmptyValue(idVal);
        }
        else if (request.Headers.TryGetValue(WebhookHeaders.DeliveryId, out var deliveryIdVal))
        {
