...
public sealed class InMemoryWebhookReplayDetector : IWebhookReplayDetector, IDisposable
{
    ...
    /// <summary>
    /// Gets the interval at which expired message identifiers are pruned.
    /// </summary>
    ...
    public InMemoryWebhookReplayDetector(TimeSpan retentionPeriod)
    {
        ...
        CleanupInterval = TimeSpan.FromTicks(Math.Max(TimeSpan.FromMinutes(1).Ticks, retentionPeriod.Ticks / 2));
    ...
    private void CleanupExpiredEntries(object? state) =>
        CleanupExpiredEntries(DateTimeOffset.UtcNow.ToUnixTimeMilliseconds());

    internal void CleanupExpiredEntries(long now)
    {
