...
public sealed class CoverageCompletenessTests
{
    ...
    private static readonly string[] WhitespaceKeyArray = ["   "];
    ...
    public async Task WebhookSenderExtensions_SendAsync_ValidatesArgumentsAndDelivers()
    {
        ...
        var exNullJson = await actNullJsonInfo.Should().ThrowAsync<ArgumentNullException>();
        exNullJson.WithParameterName("jsonTypeInfo");
        exNullJson.Which.StackTrace.Should().NotContain("JsonSerializer");
    ...
    public async Task InMemoryWebhookReplayDetector_FullLifecycle_CoversEdgeCases()
    {
        ...
        defaultDetector.CleanupInterval.Should().Be(TimeSpan.FromMinutes(2.5));

        using var tenMinDetector = new InMemoryWebhookReplayDetector(TimeSpan.FromMinutes(10));
        tenMinDetector.CleanupInterval.Should().Be(TimeSpan.FromMinutes(5));

        using var thirtySecDetector = new InMemoryWebhookReplayDetector(TimeSpan.FromSeconds(30));
        thirtySecDetector.CleanupInterval.Should().Be(TimeSpan.FromMinutes(1));
        ...
        // Eviction when capacity threshold reached (exact 100,000 items)
        seenDict.Clear();
        while (queue.TryDequeue(out _)) { }

        queue.Enqueue("dummy-0");
        seenDict.TryAdd("dummy-0", DateTimeOffset.UtcNow.AddMinutes(5).ToUnixTimeMilliseconds());
        for (var i = 1; i < 100_000; i++)
        {
            seenDict.TryAdd($"dummy-{i}", DateTimeOffset.UtcNow.AddMinutes(5).ToUnixTimeMilliseconds());
        }
        seenDict.Count.Should().Be(100_000);

        var addedOverCapacity = await defaultDetector.TryRecordAsync("id-new");
        addedOverCapacity.Should().BeTrue();
        seenDict.ContainsKey("dummy-0").Should().BeFalse();
        seenDict.ContainsKey("id-new").Should().BeTrue();

        // Cleanup expired entries with exact millisecond boundary
        seenDict["boundary-entry"] = 5000L;
        defaultDetector.CleanupExpiredEntries(5000L);
        seenDict.ContainsKey("boundary-entry").Should().BeTrue();

        defaultDetector.CleanupExpiredEntries(5001L);
        seenDict.ContainsKey("boundary-entry").Should().BeFalse();

        // Default cleanup method invoke
        ...
        seenDict.Should().BeEmpty();

        seenDict["after-dispose"] = 12345L;
        defaultDetector.Dispose(); // second call triggers if (_disposed) return;
        seenDict.ContainsKey("after-dispose").Should().BeTrue();
    ...
    public void WebhookSecret_Operations_RedactsCorrectly()
    {
        ...
        actNull.Should().Throw<ArgumentException>().WithMessage("Secret cannot be null or whitespace.*");

        var actWhitespace = () => new WebhookSecret("   ");
        actWhitespace.Should().Throw<ArgumentException>().WithMessage("Secret cannot be null or whitespace.*");
    ...
    public void SafeSocketsHttpHandlerFactory_CreateAndIsPrivateOrLoopback_CoversAllBranches()
    {
        ...
        unsecureHandler.ConnectCallback.Should().BeNull();
        ...
        secureHandler.ConnectCallback.Should().NotBeNull();
        ...
        // Specific IP boundary checks to prevent logical mutations in IsPrivateOrLoopback
        CallIsPrivate(IPAddress.Parse("8.168.1.1")).Should().BeFalse();
        CallIsPrivate(IPAddress.Parse("8.254.1.1")).Should().BeFalse();
        CallIsPrivate(IPAddress.Parse("8.20.1.1")).Should().BeFalse();
        CallIsPrivate(IPAddress.Parse("172.15.1.1")).Should().BeFalse();
    }
    ...
    public async Task SafeSocketsHttpHandlerFactory_ConnectCallback_ThrowsExpectedExceptions()
    {
        ...
        var exBlock = await actLoopback.Should().ThrowAsync<HttpRequestException>();
        exBlock.WithInnerException<System.Security.SecurityException>()
            .WithMessage("DNS Rebinding SSRF detected: The hostname '127.0.0.1' resolved to a private/loopback IP address (127.0.0.1).");
        ...
        var exRefused = await actRefused.Should().ThrowAsync<HttpRequestException>();
        exRefused.WithInnerException<SocketException>();
    }

    [Fact]
    public async Task SafeSocketsHttpHandlerFactory_ConnectCallback_SuccessfulConnection_SetsNoDelay()
    {
        ...
        var serverTask = Task.Run(async () =>
        {
            ...
            await serverSocket.SendAsync(response, SocketFlags.None);
        });

        try
        {
            var options = new WebhookSenderOptions { EnableSsrfProtection = true, AllowPrivateNetworks = true };
            using var handler = SafeSocketsHttpHandlerFactory.Create(options);
            using var client = new HttpClient(handler);

            var res = await client.GetAsync($"http://127.0.0.1:{port}/");
    ...
    [Fact]
    public async Task SafeSocketsHttpHandlerFactory_ConnectCallback_DirectInvocation_VerifiesNoDelayAndDisposal()
    {
        var listener = new TcpListener(IPAddress.Loopback, 0);
        listener.Start();
        int port = ((IPEndPoint)listener.LocalEndpoint).Port;
        ...
        {
            using var s = await listener.AcceptSocketAsync();
        });

        try
        {
            var options = new WebhookSenderOptions { EnableSsrfProtection = true, AllowPrivateNetworks = true };
            var handler = SafeSocketsHttpHandlerFactory.Create(options);

            var context = (SocketsHttpConnectionContext)System.Runtime.CompilerServices.RuntimeHelpers.GetUninitializedObject(typeof(SocketsHttpConnectionContext));
            var field = typeof(SocketsHttpConnectionContext).GetFields(BindingFlags.NonPublic | BindingFlags.Instance)
                .First(f => f.FieldType == typeof(DnsEndPoint));
            field.SetValue(context, new DnsEndPoint("127.0.0.1", port));

            using var stream = await handler.ConnectCallback!(context, CancellationToken.None);
            var netStream = (NetworkStream)stream;
            netStream.Socket.NoDelay.Should().BeTrue();
            await acceptTask;
        }
        finally
        {
    ...
    [Fact]
    public async Task SafeSocketsHttpHandlerFactory_ConnectCallback_ConnectFails_DisposesSocket()
    {
        Socket? capturedSocket = null;
        SafeSocketsHttpHandlerFactory.OnSocketCreated = s => capturedSocket = s;
        try
        {
            var options = new WebhookSenderOptions { EnableSsrfProtection = true, AllowPrivateNetworks = true };
            var handler = SafeSocketsHttpHandlerFactory.Create(options);
            ...
            var field = typeof(SocketsHttpConnectionContext).GetFields(BindingFlags.NonPublic | BindingFlags.Instance)
                .First(f => f.FieldType == typeof(DnsEndPoint));
            field.SetValue(context, new DnsEndPoint("127.0.0.1", 59999));

            var vt = handler.ConnectCallback!(context, CancellationToken.None);
            await Assert.ThrowsAnyAsync<Exception>(() => vt.AsTask());

            capturedSocket.Should().NotBeNull();
            capturedSocket!.SafeHandle.IsClosed.Should().BeTrue();
        }
        finally
        {
            SafeSocketsHttpHandlerFactory.OnSocketCreated = null;
        }
    }
    ...
    public void WebhookSenderOptions_Properties_ValidateCorrectly()
    {
        ...
        options.EnableSsrfProtection.Should().BeTrue();

        var actZeroTimeout = () => options.Timeout = TimeSpan.Zero;
        actZeroTimeout.Should().Throw<ArgumentOutOfRangeException>().WithMessage("Timeout must be greater than zero.*");

        var actNegativeTimeout = () => options.Timeout = TimeSpan.FromSeconds(-1);
        actNegativeTimeout.Should().Throw<ArgumentOutOfRangeException>().WithMessage("Timeout must be greater than zero.*");

        options.Timeout = TimeSpan.FromSeconds(30);
        options.Timeout.Should().Be(TimeSpan.FromSeconds(30));

        var actZeroPayload = () => options.MaxPayloadSizeBytes = 0;
        actZeroPayload.Should().Throw<ArgumentOutOfRangeException>().WithMessage("MaxPayloadSizeBytes must be greater than zero.*");

        var actNegativePayload = () => options.MaxPayloadSizeBytes = -100;
        actNegativePayload.Should().Throw<ArgumentOutOfRangeException>().WithMessage("MaxPayloadSizeBytes must be greater than zero.*");
    ...
    public async Task InMemoryWebhookDeadLetterSink_ValidationAndProperties()
    {
        ...
        actZero.Should().Throw<ArgumentOutOfRangeException>().WithMessage("Max capacity must be greater than zero.*");

        var actNegative = () => new InMemoryWebhookDeadLetterSink(-10);
        actNegative.Should().Throw<ArgumentOutOfRangeException>().WithMessage("Max capacity must be greater than zero.*");
    ...
    public async Task WebhookSender_ConfigurationAndMetadataValidations()
    {
        ...
        resRelative.Error.Description.Should().Be("Target URL must be an absolute URI.");
        ...
        resFtp.Error.Description.Should().Be("Target URL scheme must be http or https.");
        ...
        resHttp.Error.Description.Should().Be("Webhook delivery blocked by security policy: Insecure HTTP delivery is not permitted. Enable DangerousAllowInsecureHttp to override.");
    ...
    public async Task WebhookSigner_ComputeSignatureAsync_EdgeCases()
    {
        ...
        var expectedStandardSig = WebhookSigner.ComputeSignature(TestSecret, 1700000000L, Encoding.UTF8.GetBytes("{\"test\":true}"), WebhookSenderProtocol.Standard, webhookId: "msg-std-1");
        standardSig.Should().Be(expectedStandardSig);
        ...
        var expectedEricksonSig = WebhookSigner.ComputeSignature(TestSecret, 1700000000L, Encoding.UTF8.GetBytes("{\"test\":true}"), WebhookSenderProtocol.EricksonLopez);
        ericksonSig.Should().Be(expectedEricksonSig);
    ...
    public async Task WebhookSigner_VerifySignatureAsync_AllBranches()
    {
        ...
        // Null single secretKey overloads
        stream.Position = 0;
        var rNullKey1 = await WebhookSigner.VerifySignatureAsync((string)null!, 1700000000L, stream, "v1=abc", WebhookReceiverProtocol.AutoDetect);
        rNullKey1.Should().BeFalse();

        stream.Position = 0;
        var rNullKey2 = await WebhookSigner.VerifySignatureAsync((string)null!, 1700000000L, stream, "v1=abc", WebhookReceiverProtocol.AutoDetect, webhookId: "msg-1", maxPayloadBytes: 1000L);
        rNullKey2.Should().BeFalse();

        // Whitespace-only secretKey
        stream.Position = 0;
        var whitespaceSig = WebhookSigner.ComputeSignature("   ", 1700000000L, "{\"valid\":true}");
        var rWhitespaceKey = await WebhookSigner.VerifySignatureAsync(WhitespaceKeyArray, 1700000000L, stream, whitespaceSig, WebhookReceiverProtocol.EricksonLopez, webhookId: null, maxPayloadBytes: null);
        rWhitespaceKey.Should().BeFalse();
        ...
        // AutoDetect with invalid prefix candidate
        stream.Position = 0;
        var rAutoDetectInvalid = await WebhookSigner.VerifySignatureAsync(new[] { TestSecret }, 1700000000L, stream, "invalid_sig", WebhookReceiverProtocol.AutoDetect, webhookId: null, maxPayloadBytes: null);
        rAutoDetectInvalid.Should().BeFalse();

        // AutoDetect with Standard candidate but null webhookId
        stream.Position = 0;
        var rAutoDetectStdNoId = await WebhookSigner.VerifySignatureAsync(new[] { TestSecret }, 1700000000L, stream, "v1,someBase64Sig==", WebhookReceiverProtocol.AutoDetect, webhookId: null, maxPayloadBytes: null);
        rAutoDetectStdNoId.Should().BeFalse();

        // AutoDetect with valid Standard signature
        stream.Position = 0;
        var validStdSig = WebhookSigner.ComputeSignature(TestSecret, 1700000000L, Encoding.UTF8.GetBytes("{\"valid\":true}"), WebhookSenderProtocol.Standard, "msg-std-1");
        var rAutoDetectStdValid = await WebhookSigner.VerifySignatureAsync(new[] { TestSecret }, 1700000000L, stream, validStdSig, WebhookReceiverProtocol.AutoDetect, webhookId: "msg-std-1", maxPayloadBytes: null);
        rAutoDetectStdValid.Should().BeTrue();
    ...
    [Fact]
    public async Task SafeSocketsHttpHandlerFactory_DnsResolverEmpty_ThrowsSocketException()
    {
        SafeSocketsHttpHandlerFactory.DnsResolver = (host, ct) => Task.FromResult(Array.Empty<IPAddress>());
        try
        {
            var options = new WebhookSenderOptions { EnableSsrfProtection = true, AllowPrivateNetworks = true };
            var handler = SafeSocketsHttpHandlerFactory.Create(options);
            ...
            var field = typeof(SocketsHttpConnectionContext).GetFields(BindingFlags.NonPublic | BindingFlags.Instance)
                .First(f => f.FieldType == typeof(DnsEndPoint));
            field.SetValue(context, new DnsEndPoint("dummy.test", 80));
            ...
            ex.Which.SocketErrorCode.Should().Be(SocketError.HostNotFound);
        }
        finally
        {
            SafeSocketsHttpHandlerFactory.DnsResolver = null;
        }
    }

    [Fact]
    public async Task SafeSocketsHttpHandlerFactory_ConnectCallback_OwnsSocketTrue_DisposesUnderlyingSocket()
    {
        var listener = new TcpListener(IPAddress.Loopback, 0);
        listener.Start();
        int port = ((IPEndPoint)listener.LocalEndpoint).Port;
        ...
        try
        {
            ...
            var stream = await handler.ConnectCallback!(context, CancellationToken.None);
            capturedSocket.Should().NotBeNull();
            capturedSocket!.SafeHandle.IsClosed.Should().BeFalse();
    ...
    [Fact]
    public void WebhookSender_CreateSafeSender_WithCustomOptions_PreservesOptionsInstance()
    {
        var custom = new WebhookSenderOptions { MaxPayloadSizeBytes = 99999 };
        var sender = WebhookSender.CreateSafeSender(options: custom);
        sender.ConfiguredOptions.Should().BeSameAs(custom);
    }

    [Fact]
    public async Task WebhookSender_SendAsync_PreCancelledToken_ThrowsOperationCanceledException()
    {
        var sender = new WebhookSender(new HttpClient(), options: new WebhookSenderOptions { AllowPrivateNetworks = true });
        using var cts = new CancellationTokenSource();
        cts.Cancel();

        var msg = new WebhookMessage(new Uri("http://insecure.subscriber.test/webhook"), TestSecret, "order.created", "evt-pre-cancel", "{}");
        var act = async () => await sender.SendAsync(msg, cts.Token);
        await act.Should().ThrowAsync<OperationCanceledException>();
    }

    [Fact]
    public async Task WebhookSender_PayloadSize_StringVsBytes_BothEnforcedAtBoundary()
    {
        var sender = new WebhookSender(new HttpClient(), options: new WebhookSenderOptions { MaxPayloadSizeBytes = 10, EnableSsrfProtection = false });

        // Exact boundary (10 bytes) succeeds
        var msg11Str = new WebhookMessage(_validTarget, TestSecret, "evt", "id2", "12345678901");
        var res11Str = await sender.SendAsync(msg11Str);
        res11Str.IsFailure.Should().BeTrue();
        res11Str.Error.Code.Should().Be("WebhookSender.PayloadTooLarge");

        var msg11Bytes = new WebhookMessage(_validTarget, TestSecret, "evt", "id3", new ReadOnlyMemory<byte>(new byte[11]));
        var res11Bytes = await sender.SendAsync(msg11Bytes);
        res11Bytes.IsFailure.Should().BeTrue();
        res11Bytes.Error.Code.Should().Be("WebhookSender.PayloadTooLarge");
    }

    private sealed class DelayedMockHttpMessageHandler : HttpMessageHandler
    {
        private readonly TimeSpan _delay;
        public DelayedMockHttpMessageHandler(TimeSpan delay) => _delay = delay;
    ...
    [Fact]
    public async Task WebhookSender_DeliveryTimeout_ReturnsExpectedFormattedErrorMessage()
    {
        using var handler = new DelayedMockHttpMessageHandler(TimeSpan.FromMilliseconds(300));
        using var client = new HttpClient(handler);
        var sender = new WebhookSender(client, options: new WebhookSenderOptions { Timeout = TimeSpan.FromMilliseconds(50), EnableSsrfProtection = false });

        var msg = new WebhookMessage(_validTarget, TestSecret, "order.created", "evt-timeout", "{}");
        var res = await sender.SendAsync(msg);
        res.IsFailure.Should().BeTrue();
        res.Error.Description.Should().Contain("Delivery timeout of 0.1s exceeded.");
    }

    private sealed class InspectingMockHttpMessageHandler : HttpMessageHandler
    {
    ...
    [Fact]
    public async Task WebhookSender_ProtocolStandard_SendsStandardHeaders()
    {
        using var handler = new InspectingMockHttpMessageHandler();
        using var client = new HttpClient(handler);
        var sender = new WebhookSender(client, options: new WebhookSenderOptions { Protocol = WebhookSenderProtocol.Standard, EnableSsrfProtection = false });

        var msg = new WebhookMessage(_validTarget, TestSecret, "order.created", "msg_test_standard", "{}");
        var res = await sender.SendAsync(msg);
        res.IsSuccess.Should().BeTrue();

        handler.LastRequest.Should().NotBeNull();
        var req = handler.LastRequest!;
        req.Headers.Contains(WebhookHeaders.StandardTimestamp).Should().BeTrue();
        req.Headers.Contains(WebhookHeaders.Timestamp).Should().BeFalse();
        req.Headers.Contains(WebhookHeaders.StandardSignature).Should().BeTrue();
        req.Headers.Contains(WebhookHeaders.Signature).Should().BeFalse();
        req.Headers.Contains(WebhookHeaders.StandardEventType).Should().BeTrue();
        req.Headers.Contains(WebhookHeaders.EventType).Should().BeFalse();
        req.Headers.Contains(WebhookHeaders.StandardDeliveryId).Should().BeTrue();
        req.Headers.Contains(WebhookHeaders.DeliveryId).Should().BeFalse();
    }

    [Fact]
    public async Task WebhookSigner_ZeroMemory_And_BufferReturned_DiagnosticHooks()
    {
        byte[]? zeroedKey = null;
        byte[]? returnedBuffer = null;
        WebhookSigner.OnKeyBytesZeroed = k => zeroedKey = (byte[])k.Clone();
        WebhookSigner.OnBufferReturned = b => returnedBuffer = b;
        try
        {
            WebhookSigner.ComputeSignature("whsec_secret_diagnostic_key_123", 1000, "payload");
            zeroedKey.Should().NotBeNull();
            zeroedKey!.All(b => b == 0).Should().BeTrue();

            using var s = new MemoryStream("payload"u8.ToArray());
            await WebhookSigner.ComputeSignatureAsync("whsec_secret_diagnostic_key_123", 1000, s, WebhookSenderProtocol.EricksonLopez);
            returnedBuffer.Should().NotBeNull();

            returnedBuffer = null;
            using var sVerify = new MemoryStream("payload"u8.ToArray());
            await WebhookSigner.VerifySignatureAsync("whsec_secret_diagnostic_key_123", 1000, sVerify, "v1=abc", WebhookReceiverProtocol.EricksonLopez);
            returnedBuffer.Should().NotBeNull();
        }
        finally
        {
            WebhookSigner.OnKeyBytesZeroed = null;
            WebhookSigner.OnBufferReturned = null;
        }
    }

    [Fact]
    public async Task WebhookSigner_ComputeSignatureAsync_NullSecretKey_ThrowsArgumentNullExceptionWithParamName()
    {
        var act = async () => await WebhookSigner.ComputeSignatureAsync(null!, 1000, new MemoryStream(), WebhookSenderProtocol.EricksonLopez);
        var ex = await act.Should().ThrowAsync<ArgumentNullException>();
        ex.Which.ParamName.Should().Be("secretKey");
    }

    [Fact]
    public async Task WebhookSigner_ConsecutiveSpacesAndCandidateLimits()
    {
        var payload = "{\"data\":\"hello\"}"u8.ToArray();
        var key = "whsec_candidate_test";
        var sigValid = WebhookSigner.ComputeSignature(key, 12345, "{\"data\":\"hello\"}");

        // Consecutive spaces in receivedSignature (both sync and async)
        var doubleSpacedHeader = $"v1=dummy   {sigValid}";
        WebhookSigner.VerifySignature(key, 12345, payload, doubleSpacedHeader, WebhookReceiverProtocol.AutoDetect).Should().BeTrue();

        using var stream1 = new MemoryStream(payload);
        var resDouble = await WebhookSigner.VerifySignatureAsync(new[] { key }, 12345, stream1, doubleSpacedHeader, WebhookReceiverProtocol.AutoDetect, null, null);
        resDouble.Should().BeTrue();

        // Exactly 5 candidates (4 dummy + 1 valid) returns true
        var fiveCandidates = $"v1=d1 v1=d2 v1=d3 v1=d4 {sigValid}";
        using var stream5 = new MemoryStream(payload);
        var res5 = await WebhookSigner.VerifySignatureAsync(new[] { key }, 12345, stream5, fiveCandidates, WebhookReceiverProtocol.AutoDetect, null, null);
        res5.Should().BeTrue();

        // Secret keys with empty string or whitespace are skipped safely
        using var streamKeys = new MemoryStream(payload);
        var resEmptyKeys = await WebhookSigner.VerifySignatureAsync(new[] { "", "   ", key }, 12345, streamKeys, sigValid, WebhookReceiverProtocol.AutoDetect, null, null);
        resEmptyKeys.Should().BeTrue();
    }

    [Fact]
    public async Task WebhookSigner_ProtocolMismatch_ReturnsFalse()
    {
        var payload = "{\"data\":\"protocol\"}"u8.ToArray();
        var key = "whsec_protocol_test";
        var sigErickson = WebhookSigner.ComputeSignature(key, 12345, "{\"data\":\"protocol\"}", WebhookSenderProtocol.EricksonLopez);
        var sigStandard = WebhookSigner.ComputeSignature(key, 12345, "{\"data\":\"protocol\"}", WebhookSenderProtocol.Standard, "msg_id_1");

        // Calling VerifySignature with Protocol Standard on Erickson signature returns false
        WebhookSigner.VerifySignature(key, 12345, payload, sigErickson, WebhookReceiverProtocol.Standard, "msg_id_1").Should().BeFalse();
        // Calling VerifySignature with Protocol EricksonLopez on Standard signature returns false
        WebhookSigner.VerifySignature(key, 12345, payload, sigStandard, WebhookReceiverProtocol.EricksonLopez).Should().BeFalse();

        // Async stream overload with Protocol Standard on Erickson signature returns false
        using var s1 = new MemoryStream(payload);
        (await WebhookSigner.VerifySignatureAsync(new[] { key }, 12345, s1, sigErickson, WebhookReceiverProtocol.Standard, "msg_id_1", null)).Should().BeFalse();

        // Async stream overload with Protocol EricksonLopez on Standard signature returns false
        using var s2 = new MemoryStream(payload);
        (await WebhookSigner.VerifySignatureAsync(new[] { key }, 12345, s2, sigStandard, WebhookReceiverProtocol.EricksonLopez, null, null)).Should().BeFalse();

        // AutoDetect with unknown candidate prefix triggers line 450 fallback and returns false without hashing
        var hashCreated = false;
        WebhookSigner.OnHashCreated = _ => hashCreated = true;
        try
        {
            using var s3 = new MemoryStream(payload);
            var resUnknown = await WebhookSigner.VerifySignatureAsync(new[] { key }, 12345, s3, "unknown_candidate", WebhookReceiverProtocol.AutoDetect, null, null);
            resUnknown.Should().BeFalse();
            hashCreated.Should().BeFalse();
        }
        finally
        {
    ...
    [Fact]
    public async Task WebhookSigner_BufferPool_ReturnsBuffersInComputeAndVerifyAsync()
    {
        var pool = new TrackingArrayPool();
        WebhookSigner.BufferPool = pool;
        try
        {
            using var s1 = new MemoryStream("payload"u8.ToArray());
            await WebhookSigner.ComputeSignatureAsync("whsec_key", 1000, s1, WebhookSenderProtocol.EricksonLopez);
            pool.ReturnCount.Should().Be(1);

            using var s2 = new MemoryStream("payload"u8.ToArray());
            await WebhookSigner.VerifySignatureAsync("whsec_key", 1000, s2, "v1=abc", WebhookReceiverProtocol.EricksonLopez);
            pool.ReturnCount.Should().Be(2);
        }
        finally
        {
            WebhookSigner.BufferPool = null!;
        }
    }
}
