...
public sealed class WebhookReceiverMiddlewareAndExtensionsTests
{
    ...
    [Fact]
    public async Task InvokeAsync_ValidationFails_PayloadTooLarge_ShortCircuitsWith413AndJsonPayload()
    {
        var nextCalled = false;
        RequestDelegate next = _ =>
        {
        ...
        using var doc = JsonDocument.Parse(responseText);
        doc.RootElement.GetProperty("code").GetString().Should().Be("WebhookValidator.PayloadTooLarge");
        doc.RootElement.GetProperty("error").GetString().Should().Be("Payload exceeded 262144 bytes limit.");
    }
    ...
    public async Task InvokeAsync_WithCustomLogger_LogsWarningOnFailure()
    {
        RequestDelegate next = _ => Task.CompletedTask;
        var options = Options.Create(new WebhookReceiverOptions { RoutePath = "/api/webhooks" });
        var logger = new TestLogger<WebhookReceiverMiddleware>();
        var middleware = new WebhookReceiverMiddleware(next, options, logger);
        var validator = new FakeWebhookValidator
        {
            ResultToReturn = Error.Unauthorized("WebhookValidator.InvalidSignature", "Signature verification failed.")
        };
        ...
        logger.Messages.Should().ContainSingle()
            .Which.Should().Contain("Inbound webhook to '/api/webhooks' was rejected: [WebhookValidator.InvalidSignature] Signature verification failed.");
    }

    [Theory]
    [InlineData(0)]
    [InlineData(-1)]
    public void AddWebhookReceiver_InvalidMaxPayloadSizeBytes_ThrowsOptionsValidationException(int size)
    {
        var services = new ServiceCollection();
        services.AddWebhookReceiver(opts =>
        {
            var field = typeof(WebhookReceiverOptions).GetField("_maxPayloadSizeBytes", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
            field!.SetValue(opts, size);
        });

        using var provider = services.BuildServiceProvider();
        var act = () => provider.GetRequiredService<IOptions<WebhookReceiverOptions>>().Value;
    ...
    [Fact]
    public void AddWebhookReceiver_ZeroTimestampTolerance_ThrowsOptionsValidationException()
    {
        var services = new ServiceCollection();
        services.AddWebhookReceiver(opts => opts.TimestampTolerance = TimeSpan.Zero);

        using var provider = services.BuildServiceProvider();
        var act = () => provider.GetRequiredService<IOptions<WebhookReceiverOptions>>().Value;

        act.Should().Throw<OptionsValidationException>()
            .WithMessage("*TimestampTolerance must be greater than zero.*");
    }

    [Theory]
    [InlineData("")]
    [InlineData(null)]
    public void AddWebhookReceiver_EmptyRoutePath_ThrowsOptionsValidationException(string? routePath)
    {
        var services = new ServiceCollection();
        services.AddWebhookReceiver(opts => opts.RoutePath = routePath!);

        using var provider = services.BuildServiceProvider();
        var act = () => provider.GetRequiredService<IOptions<WebhookReceiverOptions>>().Value;

        act.Should().Throw<OptionsValidationException>()
            .WithMessage("*RoutePath must not be empty.*");
    }
    ...
    public void AddWebhookReceiver_NullServices_ThrowsArgumentNullException()
    {
        ...
        var ex = act.Should().Throw<ArgumentNullException>().WithParameterName("services").Subject.Single();
        ex.StackTrace.Should().NotContain("OptionsServiceCollectionExtensions");

        var actBothNull = () => WebhookAspNetCoreExtensions.AddWebhookReceiver(null!, null!);
    ...
    public void AddInMemoryReplayDetector_DefaultRetention_RegistersDetectorSingleton()
    {
        ...
        var field = typeof(InMemoryWebhookReplayDetector).GetField("_retentionPeriod", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        var retention = (TimeSpan)field!.GetValue(detector)!;
        retention.Should().Be(TimeSpan.FromMinutes(5));
    }
    ...
    public void AddInMemoryReplayDetector_ExplicitRetention_RegistersDetectorSingleton()
    {
        ...
        var field = typeof(InMemoryWebhookReplayDetector).GetField("_retentionPeriod", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        var retention = (TimeSpan)field!.GetValue(detector)!;
        retention.Should().Be(TimeSpan.FromMinutes(10));
    }
}
